...
class TaskAnalysis
{
    ...
    {
        $sql = "INSERT INTO " . $this->table . " (user_task_id, is_task_done, time_taken_in_hours, article_watched, video_watched, books_read) 
                VALUES (?, ?, ?, ?, ?, ?)";

        $stmt = $this->conn->prepare($sql);

        // Bind parameters
        $stmt->bind_param('iiibbb', $this->user_task_id, $this->is_task_done, $this->time_taken_in_hours, $this->article_watched, $this->video_watched, $this->books_read);
    ...
    {
        $sql = "SELECT analysis_id, user_task_id, is_task_done, time_taken_in_hours, article_watched, video_watched, books_read FROM " . $this->table;
    ...
    {
        $sql = "SELECT analysis_id, user_task_id, is_task_done, time_taken_in_hours, article_watched, video_watched, books_read FROM " . $this->table . " WHERE analysis_id = ?";
        $stmt = $this->conn->prepare($sql);
        ...
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($this->analysis_id, $this->user_task_id, $this->is_task_done, $this->time_taken_in_hours, $this->article_watched, $this->video_watched, $this->books_read);
        $stmt->fetch();
    }
    ...
    {
        $sql = "SELECT COUNT(*) as total FROM " . $this->table . " WHERE user_task_id=?";
        $stmt = $this->conn->prepare($sql);
